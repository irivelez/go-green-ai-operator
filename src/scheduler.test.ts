...
import { generateSlots, availableSlots, bookSlot, noSlotsInWindow, resetSlots } from "./scheduler";
...
async function main() {
  console.log("\n=== Scheduler 1: GENERATION (Sun 2026-06-14 → first Thu 2026-06-18) ===");
  {
    ...
    ok("first slot startTime is local ISO 08:00", slots[0]?.startTime === "2026-06-18T08:00:00", slots[0]?.startTime);
    ok("first slot endTime is local ISO 10:00", slots[0]?.endTime === "2026-06-18T10:00:00", slots[0]?.endTime);
    ...
    ok(`count === ${dayCount} days × 4 = ${dayCount * 4} (got ${slots.length})`, slots.length === dayCount * 4);
  ...
  console.log("\n=== Scheduler 2: BOOK + TAKEN (different lead, same slot) ===");
  {
    ...
    ok("returned slot.slotId matches", r1.ok === true && r1.slot.slotId === "2026-06-18-T1");
  ...
  console.log("\n=== Scheduler 3: OUT OF WINDOW ===");
  {
    ...
    ok("2099 slot → out_of_window", r.ok === false && r.reason === "out_of_window", JSON.stringify(r));
  }
  ...
  console.log("\n=== Scheduler 3b: LEAD MISSING ===");
  {
    ...
    ok("unknown lead → lead_missing", r.ok === false && r.reason === "lead_missing", JSON.stringify(r));
  }
  ...
  console.log("\n=== Scheduler 4: IDEMPOTENT (same lead, same slot, twice) ===");
  {
    ...
    ok("second call returns same slot", r1.ok && r2.ok && r1.slot.slotId === r2.slot.slotId);
    ...
    ok("ledger has 1 booking (total − available === 1)", total - avail === 1, `total=${total} avail=${avail}`);
  }
