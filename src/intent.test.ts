...
// Test 1: intent param with embedded frequency
{
  ...
  ok(
    "weekly_mowing → service + frequency",
...
// Test 4: empty params
{
  ...
  ok("empty params → empty object", Object.keys(result).length === 0, `got ${JSON.stringify(result)}`);
}
...
// Test 5: garbage input
{
  ...
  ok(
    "garbage input → raw only, no service",
...
// Test 6: mow alias
{
  ...
  ok("mow → mowing service", result.service === "mowing" && result.raw === "mow", `got ${JSON.stringify(result)}`);
}
...
// Test 9: biweekly_mowing
{
  ...
  ok(
    "biweekly_mowing → service + frequency",
...
// Test 10: monthly frequency
{
  ...
  ok("freq: monthly → frequency", result.frequency === "monthly", `got ${JSON.stringify(result)}`);
}

// Test 11: priority: intent > utm_content > svc
{
  const result = decodeIntent({ intent: "mowing", utm_content: "cleanup", svc: "recurring" });
  ok("intent takes priority over utm_content and svc", result.service === "mowing", `got ${JSON.stringify(result)}`);
}

// Test 12: utm_content > svc
{
  const result = decodeIntent({ utm_content: "cleanup", svc: "recurring" });
  ok("utm_content takes priority over svc", result.service === "cleanup", `got ${JSON.stringify(result)}`);
}
