...
export async function tool_book_evaluation(
  lead: Lead,
  slotISO: string,
): Promise<{ ok: boolean; reason?: string; lead?: Lead }> {
...
export async function tool_create_work_order(lead_id: string): Promise<Lead | { error: string }> {
  ...
  return upsertLead({
    ...
    work_order: {
      address: lead.address,
      zone: lead.zone,
      frequency: lead.desired_frequency,
      package: lead.suggested_package,
...
export async function tool_raise_escalation(
  lead_id: string,
  channel: Lead["channel"],
  reason: string,
  brief: string,
