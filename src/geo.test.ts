...
async function main() {
  console.log("\n=== Scenario 1: validateAddress → CORRECTED (didYouMean set) ===");
  {
    ...
    if (res.ok) {
      ...
      ok(
        "didYouMean populated",
        res.didYouMean === "742 Valencia St, San Francisco, CA 94110, USA",
        res.didYouMean ?? "",
      );
      ok(
        "standardized lat/lng",
  ...
  console.log("\n=== Scenario 2: autoMeasureRoofBbox → Solar payload mapped ===");
  {
    ...
    mockFetch((url) => {
      if (url.includes("solar.googleapis.com")) {
        return {
          ...
          body: {
            boundingBox: {
              sw: { latitude: 37.7595, longitude: -122.422 },
              ne: { latitude: 37.7605, longitude: -122.421 },
            },
    ...
    if (res.ok) {
      ...
      ok("bbox sw mapped", res.roof_bbox.sw.lat === 37.7595 && res.roof_bbox.sw.lng === -122.422);
      ok("bbox ne mapped", res.roof_bbox.ne.lat === 37.7605 && res.roof_bbox.ne.lng === -122.421);
    }
    restoreFetch();
  }
  ...
  console.log("\n=== Scenario 7: measureFromAddress → single-family parcel outline ===");
  {
    ...
    if (res.ok) {
      ...
      ok(
        "parcel ring returned (lat/lng)",
        Array.isArray(res.parcel_ring) && res.parcel_ring.length >= 3,
        `len ${res.parcel_ring?.length}`,
      );
      ok(
        "ring point is {lat,lng}",
        res.parcel_ring[0]?.lat === 37.75 && res.parcel_ring[0]?.lng === -122.42,
  ...
  console.log("\n=== Scenario 8: measureFromAddress → condo (mapblklot ≠ blklot) → shared_multi_unit ===");
  {
    ...
    if (res.ok) {
      ...
      ok(
        "no parcel ring needed for escalated condo",
        !res.parcel_ring || res.parcel_ring.length === 0,
  ...
  console.log("\n=== Scenario 11: validateAddress → surfaces USPS-normalized structured parts (parcel join input) ===");
  {
    ...
    if (res.ok) {
      ...
      ok(
        "parts.streetName 'OCTAVIA' (type stripped, uppercased)",
        res.parts?.streetName === "OCTAVIA",
        res.parts?.streetName,
      );
  ...
  );
  {
    ...
    const res = await validateAddress({
      addressLines: ["100 s van ness ave"],
      locality: "san francisco",
      adminArea: "CA",
      postalCode: "94103",
    });
    ...
    if (res.ok) {
      ok(
        "streetName 'SOUTH VAN NESS' (full directional kept, matches EAS)",
        res.parts?.streetName === "SOUTH VAN NESS",
        res.parts?.streetName,
      );
