...
// async main() wrapper — tsx treats .ts as CJS so top-level await isn't available;
// wrapping matches the agent-tools.test.ts / agent-route.test.ts pattern. The
// resetStore call moves inside so every run is hermetic regardless of entry point.
async function main() {
  resetStore([]);
  ...
  console.log("\n=== Scenario 1: happy A-lead (medium yard, biweekly, SF 94110) ===");
  {
    const lead = await upsertLead({
      ...
      channel: "telegram",
      name: "Dana",
      address: "742 Valencia St, San Francisco, 94110",
    ...
    ok(
      "idempotent re-book blocked",
      !(await tool_book_evaluation({ ...lead, address: lead.address } as never, "2026-06-15T15:00:00Z")).ok,
    );
  }
  ...
  console.log("\n=== Scenario 2: HOA → escalation (no autonomous booking) ===");
  {
    await upsertLead({
      ...
      channel: "telegram",
      name: "Sam",
      address: "1 Main St, San Francisco, 94105",
      photos: [],
    });
    ...
    const lead = await tool_raise_escalation(
      "L2",
      "telegram",
      esc.reasons.join(", "),
      "HOA common-area request — needs human.",
    );
  ...
  {
    const noAddr = await upsertLead({ lead_id: "L3", channel: "telegram", name: "Pat", photos: ["p.jpg"] });
    const booked = await tool_book_evaluation(noAddr, "2026-06-16T17:00:00Z");
  ...
  console.log("\n=== Scenario 4: events log captures owner corrections (HITL learning loop) ===");
  {
    await upsertLead({
      ...
      channel: "telegram",
      name: "Mo",
      address: "1500 Page St, San Francisco, 94117",
      estimated_sqft: 3800,
      area_source: "auto",
      area_confidence: 0.62,
    });
    const e1 = await appendEvent("L4", {
      actor: "owner",
      action: "override_area",
      reason_code: "area_wrong",
      corrected_value: 4200,
      agent_decision: { estimated_sqft: 3800 },
    });
    ok("appendEvent returns event with ts", typeof e1.ts === "string" && e1.ts.length > 0);
    const events = await listEvents("L4");
    ...
    await appendEvent("L4", { actor: "agent", action: "rescore", inputs: { sqft: 4200 } });
    const events2 = await listEvents("L4");
    ok("order preserved on second append", events2.length === 2 && events2[1]!.action === "rescore");

    const lead = await getLead("L4");
    ok("extended fields persist on Lead", lead?.estimated_sqft === 3800 && lead?.area_source === "auto");

    const noLead = await appendEvent("L_missing", { actor: "system", action: "noop" });
    ok(
      "missing lead no-op returns unsaved event",
      typeof noLead.ts === "string" && (await listEvents("L_missing")).length === 0,
    );
  }
