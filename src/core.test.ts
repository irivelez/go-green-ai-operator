...
{
  const lead = await upsertLead({ lead_id: "L1", channel: "telegram", name: "Dana",
...
{
  await upsertLead({ lead_id: "L2", channel: "telegram", name: "Sam",
...
{
  await upsertLead({ lead_id: "L4", channel: "telegram", name: "Mo",
    address: "1500 Page St, San Francisco, 94117", estimated_sqft: 3800,
    area_source: "auto", area_confidence: 0.62 });
  const e1 = await appendEvent("L4", { actor: "owner", action: "override_area",
  ...
  ok("missing lead no-op returns unsaved event", typeof noLead.ts === "string" && (await listEvents("L_missing")).length === 0);
}
