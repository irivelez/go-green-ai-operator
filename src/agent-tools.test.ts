...
async function main() {
  ...
  console.log("\n=== S4: recommend_tier — server re-derives from PRICE_BOOK, ignores any LLM-supplied price ===");
  {
    ...
    ok(
      "perVisit comes from PRICE_BOOK ($299) not the model",
      r.perVisit === PRICE_BOOK.signature.perVisit && r.perVisit === 299,
      `got ${r.perVisit}`,
    );
    ...
    ok(
      "options carry authoritative prices",
      r.options.every((o) => o.perVisit === PRICE_BOOK[o.tier].perVisit),
    );
  ...
  console.log("\n=== S3: propose_checkout — address gate + photos gate + NEVER charges ===");
  {
    ...
    ok(
      "missing address → refusal status, no Stripe URL",
      noAddr.status === "missing_address" && !("url" in noAddr),
    ...
    ok(
      "photos+address present → not a gate refusal",
      ready.status !== "missing_address" && ready.status !== "missing_photos",
    ...
    ok(
      "computes authoritative amount",
      typeof ready.amount === "number" && approxEq(ready.amount as number, expectAmount),
      `got ${ready.amount}`,
    );
    ok(
      "dev (no Stripe key) → checkout_unavailable_dev, NEVER a silent charge",
      ready.status === "checkout_unavailable_dev",
    );
  }
  ...
  console.log("\n=== confirm_booking — payment gate (no booking before paid) ===");
  {
    ...
    ok(
      "offer_slots returns availability",
      Array.isArray(slots) && slots.length > 0,
      `got ${Array.isArray(slots) ? slots.length : typeof slots}`,
    );
    const firstSlot = slots[0]!.slotId;
    const blocked = await runConfirmBooking(ctx("L5"), { slotId: firstSlot });
    ok(
      "unpaid lead → booking refused (payment_required)",
      blocked.status === "payment_required",
    ...
    ok(
      "Ready-to-Schedule WITHOUT paid_at (operator/hitl path) → still payment_required",
      noProof.status === "payment_required",
    ...
    ok(
      "lead moved to Scheduled",
      (await getLead("L5"))?.status === "Scheduled" || (await getLead("L5"))?.status === "Work Order Created",
    );
  }
  ...
  console.log("\n=== raise_escalation — marks lead, blocks auto-charge ===");
  {
    ...
    ok(
      "lead status → Needs Human Review",
      (await getLead("L6"))?.status === "Needs Human Review",
      (await getLead("L6"))?.status,
    );
  }
  ...
  );
  {
    ...
    ok(
      "parts persisted on CORRECTED (Yes button doesn't re-validate)",
      lead?.street_name === "OCTAVIA",
      lead?.street_name,
    );
  ...
  console.log("\n=== T10.a5: validate_address — re-edit overwrites prior parts (No → re-enter) ===");
  {
        ...
        return {
          ...
          body: {
            result: {
              verdict: { addressComplete: true },
              address: {
                formattedAddress: "566 South Van Ness Ave, San Francisco, CA 94110, USA",
                addressComponents: [
    ...
    await runValidateAddress(ctx("L7e"), {
      addressLines: ["566 South Van Ness Ave"],
      locality: "San Francisco",
      adminArea: "CA",
      postalCode: "94110",
    });
      ...
      if (url.includes("addressvalidation.googleapis.com")) {
        return {
          ...
          body: {
            result: {
              verdict: {},
              address: { formattedAddress: "asdf" },
              geocode: { location: { latitude: 0, longitude: 0 } },
            },
          },
        };
      }
  ...
  console.log("\n=== T10.g1: measure_property — slope read from lead lat/lng, not LLM args (steep stays steep) ===");
  {
    ...
    ok(
      "elevation sampled the lead's REAL lat (37.79), not LLM 0,0",
      elevationSeen.includes("37.79"),
      elevationSeen.slice(0, 90),
    );
    ok(
      "lead.slope_tier persisted steep",
      (await getLead("L_M3"))?.slope_tier === "steep",
      (await getLead("L_M3"))?.slope_tier,
    );
  }
  ...
  console.log("\n=== T10.b: confirm_area — re-derives sqft from path; client-supplied number ignored ===");
  {
    ...
    if (r.status === "confirmed") {
      ok(
        "re-derived confirmed_sqft ≈ 53820 (±200)",
        Math.abs(r.confirmed_sqft - 53820) < 200,
        `got ${r.confirmed_sqft}`,
      );
  ...
  console.log("\n=== T10.b2: confirm_area — no lead → lead_missing, NEVER fabricates a flat-slope stub ===");
  {
    ...
    ok(
      "confirmed_sqft still echoed for client feedback",
      typeof (r as { confirmed_sqft?: number }).confirmed_sqft === "number",
    );
    ok(
      "lead NOT created with default flat slope",
      (await getLead("L_NOLEAD")) === undefined,
      JSON.stringify(await getLead("L_NOLEAD")),
    );
  }
  ...
  console.log("\n=== T10.b3: confirm_area → compute_exact_price — colocated steep slope prices steep (not flat) ===");
  {
    ...
    ok(
      "confirm_area preserves steep",
      ca.status === "confirmed" && ca.slope_tier === "steep",
      JSON.stringify(ca).slice(0, 120),
    );
    ...
    ok(
      "price uses steep multiplier (not flat)",
      price.status === "priced" && price.perVisit === expected.perVisit,
      JSON.stringify(price).slice(0, 120),
    );
  }
  ...
  console.log("\n=== T10.c: confirm_area — vision steepness_hint='steep' raises tier flat→moderate ===");
  {
    ...
    if (r.status === "confirmed") {
      ...
      ok(
        "persisted to lead",
        (await getLead("L9"))?.slope_tier === "moderate" && (await getLead("L9"))?.slope_source === "photo_raised",
      );
    }
  ...
  console.log("\n=== SEC-F: confirm_area re-draw clears stale price (no pay against an outdated quote) ===");
  {
    ...
    ok(
      "stale per_visit_price cleared on re-confirm",
      lead?.per_visit_price === undefined,
  ...
  console.log("\n=== SEC-D: confirm_area — out-of-range polygon is refused, never persisted as confirmed ===");
  {
    ...
    ok(
      "out-of-range returns status 'area_out_of_range'",
      r.status === "area_out_of_range",
      JSON.stringify(r).slice(0, 200),
    );
    ok(
      "confirmed_sqft echoed for client feedback",
      typeof r.confirmed_sqft === "number" && r.confirmed_sqft > 60000,
      `got ${r.confirmed_sqft}`,
    );
    const lead = await getLead("L_SEC_D");
    ok(
      "lead NOT marked area_confirmed_by_customer",
      lead?.area_confirmed_by_customer !== true,
      `got ${lead?.area_confirmed_by_customer}`,
    );
    ok(
      "lead.confirmed_sqft NOT persisted from oversized polygon",
  ...
  console.log("\n=== SEC-E: analyze_photos — LLM-supplied unsafe photoUrls are not persisted to the lead ===");
  {
    ...
    ok(
      "metadata-IP url NOT persisted to lead.photos",
      !photos.some((p) => p.startsWith("http")),
      JSON.stringify(photos).slice(0, 120),
    );
    ok(
      "safe data: url retained",
      photos.some((p) => p.startsWith("data:image/")),
    );
  }
  ...
  console.log("\n=== T10.d: compute_exact_price — missing confirmed_sqft → structured refusal ===");
  {
    ...
    ok(
      "message present",
      typeof (r as { message?: string }).message === "string" && (r as { message: string }).message.length > 0,
    );
  }
  ...
  console.log("\n=== T10.e: compute_exact_price — 2500 sqft + flat + biweekly matches pricePerVisit ===");
  {
    ...
    if (r.status === "priced") {
      ok(
        "perVisit matches engine",
        r.perVisit === expected.perVisit,
        `got ${r.perVisit}, expected ${expected.perVisit}`,
      );
      ok(
        "monthly matches engine",
        Math.abs(r.monthly - expected.monthly) < 0.005,
        `got ${r.monthly}, expected ${expected.monthly}`,
      );
  ...
  );
  {
    ...
    ok("not a gate refusal", r.status !== "missing_address" && r.status !== "missing_photos", JSON.stringify(r));
  ...
  console.log("\n=== T13.c: compute_exact_price — persists per_visit_price + monthly_price on the lead ===");
  {
    ...
    ok("per_visit_price persisted (173)", lead?.per_visit_price === 173, `got ${lead?.per_visit_price}`);
    ok(
      "monthly_price persisted (375.41)",
      typeof lead?.monthly_price === "number" && approxEq(lead.monthly_price, 375.41),
      `got ${lead?.monthly_price}`,
    );
    ok("desired_frequency persisted", lead?.desired_frequency === "biweekly", `got ${lead?.desired_frequency}`);
    ok(
  ...
  );
  {
    ...
    ok(
      "lead status Scheduled (booking persisted)",
