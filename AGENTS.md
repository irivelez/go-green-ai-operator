...
- **Cross-route store coherence (§2)** — the in-memory backend is a per-module-instance singleton, so separate Next.js route handlers (`/api/funnel/agent` vs `/api/funnel/confirm-area`) do **not** share lead state on Vercel (and not reliably across compiled modules in dev). The `measure → confirm → price` chain then prices a **stale slope** (a steep SF lot quotes as flat) because `confirm_area` can't see the slope `measure_property` wrote. **Mitigated in dev** by `STORE_BACKEND=json` (file-backed → all routes read one source of truth) plus a `lead_missing` guard in `runConfirmArea` (refuses to fabricate a default-flat-slope stub; surfaces 409 instead of a silent wrong price). **Blocks:** any Vercel-prod cross-invocation flow. **First step:** a `KvBackend`/Redis adapter behind the existing `Backend` interface, switched via `STORE_BACKEND=kv`.
