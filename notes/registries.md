...
| LLM tool names (`qualify_lead`, `analyze_photos`, `validate_address`, `measure_property`, `confirm_area`, `recommend_tier`, `compute_pricing`, `compute_exact_price`, `propose_checkout`, `offer_slots`, `confirm_booking`, `raise_escalation`) | Prompt/eval contract | `agent-tools.ts` `buildTools` | Named in `funnel-agent-prompt.ts` + evals; a rename needs the **3-sync** (spec/code+tests/prompt) — treat as frozen here |
